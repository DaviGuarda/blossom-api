...
export const createTask = async (request: AuthRequest, response: Response) => {
    try {
        const userId = request.user
        const { name, date, categoryId}:ITaks = request.body
...
export const getAllTasksByCategory =async (request: AuthRequest, response: Response) => {
    try {
        ...
        response.send({error: "Error while fetching tasks by category"})
        throw error
    }
}
...
export const editTask = async (request: AuthRequest, response: Response) => {
    try {
        const { _id, categoryId, date, name }:ITaks = request.body
        ...
        response.send({error: "Error while updating task"})
        throw error
    }
}